...
def ridgeplot(data, feature, path_):
    ...
    # color = iter(cm.rainbow(np.linspace(0, 1, len(classes))))
    plt.figure(dpi=800)
    fig6, axes = joypy.joyplot(data, by="target", figsize=(7, 5), overlap=4, bins=10, linecolor="black",
...
def kdeplotsplit(data, feature, path_):
    ...
    fig, axes = plt.subplots(1, int(len(classes)), figsize=(10, 4),
                             dpi=800, sharex=True, sharey=True)
